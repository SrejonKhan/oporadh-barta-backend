...
export const createCrimeReport = async ({
  ...
  try {
    ...
    // Create location
    const location = await createLocation(division, district, fullAddress);

    // Upload media files
    const mediaUrls = await Promise.all(
      mediaFiles.map(file => uploadMedia(
        file,
    ...
    if (!hasVideo && !description) {
      try {
        ...
        throw new AppError(httpStatus.SERVICE_UNAVAILABLE, "Failed to generate description. Please provide a manual description.");
      }
    }
